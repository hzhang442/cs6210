...
/* Random queue mapping random number to ID */
static indexrndq_t id_by_rnd;
...
int gtcache_init(size_t capacity, size_t min_entry_size, int num_levels){
  ...
  indexrndq_init(&id_by_rnd, cache.max_entries);
...
int gtcache_set(char *key, void *value, size_t val_size){
  ...
  while (need_eviction(val_size)) {
    ...
    if (!indexrndq_isempty(&id_by_rnd)) {
      id = indexrndq_dequeue(&id_by_rnd);
  ...
  *idp = id;
  ...
  /* Add ID to minpq */
  indexrndq_enqueue(&id_by_rnd, id);
